...
text_folder = '/home/rettenls/data/texts/coha/random/'
exp_folder = '/home/rettenls/data/experiments/coha/random/'
...
for model in models:
	for batch in batches:
		...
		for run_number in range(max_run_num):
			...
			if (not os.path.exists(run)) or (len(os.listdir(run)) < 3):
				...
				if not run_in_progress(run + "_RUN"):
					...
					# Train & Save
					m = Model(model)
					text_file = text_folder + 'shuffle/' + batch + '/run_{:04d}.txt'.format(run_number)
					m.train(text_file)
					m.save(run)
		...
		# MERGE RUNS
		#---------------------------------------------------------------------------------------------------
		if (len([f for f in os.listdir(folder) if (len(os.listdir(folder + '/' + f)) > 2)]) >= max_run_num + 15):
