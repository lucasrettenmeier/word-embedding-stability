...
cor = np.zeros((len(models), len(languages), 2, len(n_values), len(n_values)))
total = np.zeros((len(models), 2, len(n_values), len(n_values)))
			...
			for i in range(2):
				for j in range(len(n_values)):
					for k in range(len(n_values)):
						if (j > k):
							cor[model_index,lang_index,i,j,k] = 0
						if (j == k):
							cor[model_index,lang_index,i,j,k] = 1
						if (j  <  k):
							l = k - j - 1
							cor[model_index,lang_index,i,j,k] = results[i][j][l]
